...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    if (!loading && (!user || userData?.role !== "Church Admin")) {
      // router.push("/login"); // Commented out for now until Auth is fully verified
    }
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        <div className="flex flex-col">
          <h1 className="text-lg font-bold text-slate-900">{userData?.churchName || "Church Admin"}</h1>
          <p className="text-xs text-slate-500">{userData?.name || user?.email}</p>
        </div>
